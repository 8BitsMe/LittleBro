...
GetOCRArea(tlX, tlY, brX, brY, options="") {
     ...
     DrawRect(topLeftX,topLeftY,topLeftX+widthToScan,topLeftY+heightToScan,"00FFFF")
...
WaitForColor(Why,X,Y,Color,Timeout)
{
     ...
     DrawRect(X-6,Y-6,X+6,Y+6,"FF00FF")
          ...
          for k, v in tier {
               If InStr(param, k){
					if (paramType <> 3) {
						...
						PixelGetColor, aColor, getXCoord(0.853), getYCoord(0.422)
						If ( aColor <> 0XD33318 ) {
							 PixelSearch, Px, Py, getXCoord(xCoord), getYCoord(0.405), getXCoord(xCoord+3), getYCoord(0.698), 0XD33318, 10, Fast
							 ;SCROLL..
